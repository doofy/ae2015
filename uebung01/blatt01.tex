...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Blöcke 1 - 3 sind optimal
    \item Block 4 kann insg. optimiert werden
  \end{itemize}
  \vspace{1cm}
  \begin{lstlisting}[caption={Block 4 Optimierung},captionpos=b,frame=single]
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Ausnutzung der örtlichen Lokalität für $A, B, C$
    \item I/O Operationen: $\frac{n}{Blockgr} + \frac{k}{Blockgr} + \frac{n}{Blockgr} = \frac{2n+k}{Blockgr.} \Rightarrow O(\frac{n+k}{Blockgr.})$
  \end{itemize}
